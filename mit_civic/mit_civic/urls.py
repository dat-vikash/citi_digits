...
urlpatterns = patterns('',
        ...
        url(r'^popup/(?P<layer>.+)/(?P<neighborhood>.+)/(?P<perin>.+)/(?P<dol>.+)/(?P<sale>.+)/(?P<win>.+)/(?P<income>.+)/(?P<netwin>.+)/(?P<id>.+)/$','citi_digits.views.popup',name='popup'),
        url(r'^mathematical_explain/(?P<neighborhood>.+)/(?P<spent>.+)/(?P<income>.+)/$', 'citi_digits.views.mathExplain', name='math_explaination'),
        url(r'^not_all_equal/(?P<neighborhood>.+)/$', 'citi_digits.views.notAllEqual', name='not_all_equal'),
